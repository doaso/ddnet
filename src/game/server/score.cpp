...
void CScore::Register(const char *pNickname, const char *pPassword)
{
        auto Tmp = std::make_unique<CSqlUserRequest>(nullptr);
        str_copy(Tmp->m_aLogin, pNickname, sizeof(Tmp->m_aLogin));
        str_copy(Tmp->m_aPassword, pPassword, sizeof(Tmp->m_aPassword));
        m_pPool->Execute(CScoreWorker::Register, std::move(Tmp), "register account");
}

void CScore::ChangePassword(const char *pNickname, const char *pPassword)
{
        auto Tmp = std::make_unique<CSqlUserRequest>(nullptr);
        str_copy(Tmp->m_aLogin, pNickname, sizeof(Tmp->m_aLogin));
        str_copy(Tmp->m_aPassword, pPassword, sizeof(Tmp->m_aPassword));
        m_pPool->Execute(CScoreWorker::ChangePassword, std::move(Tmp), "change password");
}

void CScore::ChangeAdminLevel(const char *pNickname, std::uint8_t AdminLevel)
{
        auto Tmp = std::make_unique<CSqlUserRequest>(nullptr);
        str_copy(Tmp->m_aLogin, pNickname, sizeof(Tmp->m_aLogin));
        Tmp->m_AdminLevel = AdminLevel;
        m_pPool->Execute(CScoreWorker::ChangeAdminLevel, std::move(Tmp), "change admin level");
}

void CScore::ChangeLevel(const char *pNickname, std::uint32_t Level)
{
    auto Tmp = std::make_unique<CSqlUserRequest>(nullptr);
    str_copy(Tmp->m_aLogin, pNickname, sizeof(Tmp->m_aLogin));
    Tmp->m_Level = Level;
    m_pPool->Execute(CScoreWorker::ChangeLevel, std::move(Tmp), "change level");
}

void CScore::ChangeXP(const char *pNickname, std::uint32_t XP)
{
    auto Tmp = std::make_unique<CSqlUserRequest>(nullptr);
    str_copy(Tmp->m_aLogin, pNickname, sizeof(Tmp->m_aLogin));
    Tmp->m_XP = XP;
    m_pPool->Execute(CScoreWorker::ChangeXP, std::move(Tmp), "change XP");
}

void CScore::ChangePoints(const char *pNickname, std::uint32_t Points)
{
        auto Tmp = std::make_unique<CSqlUserRequest>(nullptr);
        str_copy(Tmp->m_aLogin, pNickname, sizeof(Tmp->m_aLogin));
        Tmp->m_Points = Points;
        m_pPool->Execute(CScoreWorker::ChangePoints, std::move(Tmp), "change points");
}

void CScore::ChangeDonateRubles(const char *pNickname, std::uint32_t DonateRubles)
{
        auto Tmp = std::make_unique<CSqlUserRequest>(nullptr);
        str_copy(Tmp->m_aLogin, pNickname, sizeof(Tmp->m_aLogin));
        Tmp->m_DonateRubles = DonateRubles;
        m_pPool->Execute(CScoreWorker::ChangeDonateRubles, std::move(Tmp), "change donate rubles");
}
